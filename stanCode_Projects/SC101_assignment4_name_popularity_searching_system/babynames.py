...
def add_data_for_name(name_data, year, rank, name):
    ...
    if name in name_data:
        ...
        if year in year_and_rank:
            old_rank = int(year_and_rank[year])
            new_rank = min(old_rank, int(rank))
            year_and_rank[year] = str(new_rank)
        else:
...
def search_names(name_data, target):
    ...
    for name in name_data:
        first_upper_names.append(name)
    lower_names = []
    ans = ''
    for first_upper_name in first_upper_names:
        ans = ''
        for ch in first_upper_name:
            ans += ch
            ans = ans.lower()
        lower_names.append(ans)
    lower_target = target.lower()
    names = []
    for lower_name in lower_names:
        if lower_target in lower_name:
            proper_name = ''
            proper_name += lower_name[0].upper()
            for k in range(1, len(lower_name)):
                proper_name += lower_name[k]
